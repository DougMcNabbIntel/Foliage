...
CPUTModel::~CPUTModel()
{
    ...
    SAFE_DELETE_ARRAY(mpSubMaterialCount);
...
//-----------------------------------------------------------------------------
void CPUTModel::SetSubMaterials(UINT ii, CPUTMaterial **pMaterial)
{
    // release previous submaterials
    CPUTMaterial **pCur = mpMaterial[ii];
    if( pCur )
    {
        while( *pCur )
        {
    ...
    mpSubMaterialCount[ii] = count;
    mpMaterial[ii] = new CPUTMaterial*[count+1];
    for( UINT jj=0; jj<count; jj++ )
    {
        mpMaterial[ii][jj] = pMaterial[jj];
        if( mpMaterial[ii][jj] )
        {
            mpMaterial[ii][jj]->AddRef();
        } else
        {
            mpMaterial[ii][jj] = NULL;
        }
    }
    mpMaterial[ii][count] = NULL;
}
