...
//--------------------------------------------------------------------------------------
CPUTTexture *CPUTTexture::CreateTextureArrayFromFilenameList( CPUTRenderParameters &renderParams, cString &name, cString **pAbsolutePathAndFilename, bool loadAsSRGB )
{
    // TODO: accept DX11/OGL param to control which platform we generate.
    // TODO: be sure to support the case where we want to support only one of them
#ifdef CPUT_FOR_DX11
    return CPUTTextureDX11::CreateTextureArrayFromFilenameList( *(CPUTRenderParametersDX*)&renderParams, name, pAbsolutePathAndFilename, loadAsSRGB );
#elif defined(CPUT_FOR_OGLES)
    return CPUTTextureOGLES::CreateTextureArrayFromFilenameList( *(CPUTRenderParametersOGLES*)&renderParams, name, pAbsolutePathAndFilename, loadAsSRGB );
#else    
    #error You must supply a target graphics API (ex: #define CPUT_FOR_DX11), or implement the target API for this file.
#endif
