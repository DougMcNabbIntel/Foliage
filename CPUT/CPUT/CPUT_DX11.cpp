    ...
    if( mpPerFrameConstantBuffer )
    {
        ...
        mpContext->Map( pBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapInfo );
        {
            ...
            if( pCamera )
            {
                ...
                pCb->InverseView     = XMMATRIX((float*)&inverseView);
            }
...
// are preserved
extern float gBillboardDistance;
//-----------------------------------------------------------------------------
    ...
    while(token) 
    {
        if('-' == token[0])
        {
            ...
            if(0!=ParameterName.size())
            {
                ...
                else if(0==ParameterName.compare(_L("distance")))
                {
                    // get the filename 
                    token = wcstok_s(NULL, separators, &nextToken);
                    gBillboardDistance = (float)_wtof(token);
                }
                else
                {
