...
CPUTResult CPUTMaterialDX11::LoadMaterial(
    ...
    if( mConfigBlock.GetValueByName( L"MultiMaterial" )->ValueAsBool() )
    {
        ...
        {
            mpSubMaterials[ii] = pAssetLibrary->GetMaterial( mpSubMaterialNames[ii], false, modelSuffix, meshSuffix, pShaderMacros );
    ...
    else
    {
        ...
        for( ii=0; ii<systemMaterialCount; ii++ )
        {
            ...
            mpSubMaterials[ii+1]     = pAssetLibrary->GetMaterial( mpSubMaterialNames[ii+1], false, modelSuffix, meshSuffix, pShaderMacros );
        }
...
CPUTMaterial *CPUTMaterialDX11::CloneMaterial(const cString &fileName, const cString &modelSuffix, const cString &meshSuffix)
{
    ...
    if( mSubMaterialCount > 0 )
    {
        ...
        for( int ii=0; ii<mSubMaterialCount; ii++ )
        {
            ...
            {
                pMaterial->mpSubMaterials[ii] = mpSubMaterials[ii]->CloneMaterial( mpSubMaterialNames[ii], modelSuffix, meshSuffix );
            }
        }
