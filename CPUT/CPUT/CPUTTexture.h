...
protected:
    ...
    UINT          mNumArraySlices;

    ~CPUTTexture(){} // Destructor is not public.  Must release instead of delete.

public:
    CPUTTexture()              : mMappedType(CPUT_MAP_UNDEFINED), mNumArraySlices(1) {}
    CPUTTexture(const cString &name, UINT numArraySlices=1) : mMappedType(CPUT_MAP_UNDEFINED), mName(name), mNumArraySlices(numArraySlices) {}
    static CPUTTexture *CreateTexture( const cString &name, const cString absolutePathAndFilename, bool loadAsSRGB );
    static CPUTTexture *CreateTextureArrayFromFilenameList( CPUTRenderParameters &renderParams, cString &name, cString **pAbsolutePathAndFilename, bool loadAsSRGB );
